    ...
    dataReady = function() {//This is called as soon as data is loaded
		...
		a.iterNodes(
			function (b) { //This is where we populate the array used for the group select box
				...
				// alert(b.x);
				//console.log(b.attr.attributes["Modularity Class"])
...
function setupGUI(config) {
	...
	if (!config.features.groupSelectorAttribute) {
		...
		$("#attributeselect1").hide();
	}
    $GP.cluster = new Cluster($GP.form.find("#attributeselect"));
	//$GP.subgenre = new Subgenre($GP.form.find("#attributeselect1"));
    config.GP=$GP;
    initSigma(config);
}
...
function configSigmaElements(config) {
	...
	var cluster = [],
        b,x=1;
		for (b in sigInst.subgenres) cluster.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> Group ' + (x++) + ' (' + sigInst.subgenres[b].length + ' members)</a></div>');
	$GP.subgenre.content(cluster.join(""));
	//console.log(cluster);
...
function Subgenre(a) {
    this.subgenre = a;
    this.display = !1;
    this.list = this.subgenre.find(".list");
    this.list.empty();
    this.select = this.subgenre.find(".select");
    this.select.click(function () {
        $GP.subgenre.toggle()
    });
    this.toggle = function () {
...
/*
function showGroups1(a) {
    a ? ($GP.intro.find("#showGroups1").text("Hide groups"), $GP.bg.show(), $GP.bg2.hide(), $GP.showgroup = !0) : ($GP.intro.find("#showGroups1").text("View Groups"), $GP.bg.hide(), $GP.bg2.show(), $GP.showgroup = !1)
}
*/
function nodeNormal() {
    !0 != $GP.calculating && !1 != sigInst.detail && (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(200).animate({width:'hide'},350),$GP.cluster.hide(), sigInst.iterEdges(function (a) {
        a.attr.color = !1;
        a.hidden = !1
    }), sigInst.iterNodes(function (a) {
        a.hidden = !1;
...
/*
function showSubgenre(a) {
    var b = sigInst.subgenres[a];
    if (b && 0 < b.length) {
        showGroups(!1);
        sigInst.detail = !0;
        b.sort();
